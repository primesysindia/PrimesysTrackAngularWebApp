...
export class ArrowDivDirective {
  constructor( private keyboardServ:KeyboardService,public element:ElementRef){}
  ...
  @HostListener('keydown', ['$event']) onKeyUp(e) {
    ...
    {
      case 38:
        this.keyboardServ.sendMessage({element:this.element,action:'UP'})
        break;
      case 37:
        if (this.element.nativeElement.selectionStart===undefined || this.element.nativeElement.selectionStart<=0)
        {
        this.keyboardServ.sendMessage({element:this.element,action:'LEFT'})
        e.preventDefault();
        }
        break;
      case 40:
        this.keyboardServ.sendMessage({element:this.element,action:'DOWN'})
        break;
      case 39:
        ...
        {
        this.keyboardServ.sendMessage({element:this.element,action:'RIGTH'})
        e.preventDefault();
        }
        break;
    }
